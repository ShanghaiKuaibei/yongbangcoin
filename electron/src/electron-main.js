...
const defaultURL = 'http://127.0.0.1:7050/';
let currentURL;
...
function startSkycoin() {
  console.log('Starting yongbangcoin from electron');

  if (skycoin) {
    console.log('Yongbangcoin already running');
  ...
  case 'darwin':
    return path.join(appPath, '../../Resources/app/yongbangcoin');
  case 'win32':
    // Use only the relative path on windows due to short path length
    // limits
    return './resources/app/yongbangcoin.exe';
  case 'linux':
    return path.join(path.dirname(appPath), './resources/app/yongbangcoin');
  default:
    return './resources/app/yongbangcoin';
  }
})()
  ...
  skycoin.on('error', (e) => {
    dialog.showErrorBox('Failed to start yongbangcoin', e.toString());
    app.quit();
  });
...
function createWindow(url) {
  ...
  ses.clearCache(function () {
    console.log('Cleared the caching of the yongbangcoin wallet.');
  });
  ...
  var template = [{
    label: "Yongbangcoin",
    submenu: [
      { label: "About Yongbangcoin", selector: "orderFrontStandardAboutPanel:" },
      { type: "separator" },
